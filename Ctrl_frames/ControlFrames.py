        ...
        self.ctrl_subtypes = [
            {
                ...
                "payload_size": 1,
                "standard_payload": binascii.unhexlify(
            ...
            while True:
                ...
                for i in self.fuzzer_state:
                    ...
                    if frame_info["frame_id"] == 6:
                        ...
                        print(
                            f'Transmitting {bcolors.OKBLUE}{frame_info["frame_name"]}{bcolors.ENDC} frames with random payload'
                        )
                        self.rotating_sym.start()
                        print("\n")
                        while True:
                            frame = self.fuzzer_state["payload"]["send_function"](
                                self.ctrl_frm_ext
                            )
                            frames_till_disr += frame
                            if self.check_conn_aliveness(frame, i):
                                init_logs.logging_conn_loss(
                                    f"Connectivity issues detected while sending {frame_info['frame_name']} frames with random {i}\nframe = {frame}\n\n",
                                    init_logs.is_alive_path_ctrl,
                                )
                                init_logs.logging_conn_loss(
                                    f"Prior to connection loss found the above frames were sent. Timestamp of logging is cycle {counter}\n",
                        ...
                        print(
                            f'Transmitting {bcolors.OKBLUE}{frame_info["frame_name"]}{bcolors.ENDC} frames with random Frame Control Flags'
                        )
                        self.rotating_sym.start()
                        print("\n")
                        while True:
                            frame = self.fuzzer_state["Frame Control Flags"][
                                "send_function"
                            ]()
                            frames_till_disr += frame
                            if self.check_conn_aliveness(frame, i):
