...
class LogFiles:
    def __init__(self):
        ...
        self.is_alive_path_mngmt = self.folder_path_mngmt + datetime.now().strftime(
            f"/Aliveness_check_%d-%m-%y__%H:%M:%S"
        )
        self.is_alive_path_ctrl = self.folder_path_ctrl + datetime.now().strftime(
            f"/Aliveness_check_%d-%m-%y__%H:%M:%S"
        )
        self.is_alive_path_data = self.folder_path_data + datetime.now().strftime(
            f"/Aliveness_check_%d-%m-%y__%H:%M:%S"
        )
        self.frames_till_disr_mngmt = self.folder_path_mngmt + datetime.now().strftime(
            f"/frames_till_disr_%d-%m-%y__%H:%M:%S"
        )
        self.frames_till_disr_ctrl = self.folder_path_ctrl + datetime.now().strftime(
            f"/frames_till_disr_%d-%m-%y__%H:%M:%S"
        )
        self.frames_till_disr_data = self.folder_path_data + datetime.now().strftime(
            f"/frames_till_disr_%d-%m-%y__%H:%M:%S"
        )
