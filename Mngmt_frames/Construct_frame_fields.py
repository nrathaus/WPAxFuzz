...
class Frame:
    ...
    def construct_RSN(self, mode):
        ...
        rsn_array = [
            bytearray(b"\x01\x00"),  # RSN Version 1
                ...
                for i in list_of_fields:
                    ...
                    if i == "empty":
                        ...
                        for _ in range(1, NUM_OF_FRAMES_TO_SEND):
                            ...
                            if self.check_conn_aliveness(frame, i):
                                init_logs.logging_conn_loss(
                                    f"Connectivity issues detected while sending {i} {self.frame_name} frames\nframe = {frame}\n\n",
                        ...
                        for _ in range(1, NUM_OF_FRAMES_TO_SEND):
                            ...
                            if self.check_conn_aliveness(frame, i):
                                init_logs.logging_conn_loss(
                                    f"Connectivity issues detected while sending {self.frame_name} frames with malformed {i}\nframe = {frame}\n\n",
                                    init_logs.is_alive_path_mngmt,
                                )
                                init_logs.logging_conn_loss(
                                    f"Prior to connection loss found the above frames were sent. Timestamp of logging is cycle {counter}\n",
